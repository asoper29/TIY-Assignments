...
function conway(cell, neighbors){
  if (cell === "true") {
    if (numberOfLiveNeighbors(neighbors) < 2) {
      return false; }
    else if (numberOfLiveNeighbors(neighbors) < 4) {
      return true; }
    else if (numberOfLiveNeighbors(neighbors) > 3) {
      return false; }
  } else {
    if (numberOfLiveNeighbors(neighbors) < 4) {
      return true; }
      else {
      return false;
    }
 ...
 * - THEN returns a list of the neighboring cells.
 ...
 function neighborsOf (row, col){
   if (row === 0 && col === 0){
     ...
     alert("Value inputed is invalid. Please try again.");
   }
}
